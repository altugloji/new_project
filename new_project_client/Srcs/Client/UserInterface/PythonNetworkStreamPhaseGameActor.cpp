...
bool CPythonNetworkStream::RecvCharacterAdditionalInfo()
{
	...
	if(kNetActorData.m_dwVID == chrInfoPacket.dwVID)
	{
		...
		if (kNetActorData.m_bType == CActorInstance::TYPE_NPC && !(kNetActorData.m_dwRace >= 30000 && kNetActorData.m_dwRace <= 30008))
		{
			const char* c_szName{};
			if (CPythonNonPlayer::Instance().GetName(kNetActorData.m_dwRace, &c_szName))
			{
				...
				{
					if (!kNetActorData.m_stName.empty() && kNetActorData.m_stName[0] == '#')
						kNetActorData.m_stName.erase(0, 1);
					else
						kNetActorData.m_stName = c_szName;
				}
			}
...
bool CPythonNetworkStream::RecvCharacterUpdatePacket()
{
	...
	kNetUpdateActorData.m_dwLastPlaytime = chrUpdatePacket.dwLastPlaytime;
#endif
...
void CPythonNetworkStream::__RecvCharacterUpdatePacket(SNetworkUpdateActorData * pkNetUpdateActorData)
{
	...
	if (rkPlayer.IsMainCharacterIndex(pkNetUpdateActorData->m_dwVID))
	{
		...
		#ifdef NEW_SELECT_CHARACTER
		m_akSimplePlayerInfo[m_dwSelectedCharacterIndex].dwLastPlaytime = pkNetUpdateActorData->m_dwLastPlaytime;
		#endif
	}
	else
	{
