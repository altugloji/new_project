...
bool CPythonSystem::GetDynamicShadow()
{
	return m_Config.bDynamicShadow;
}

void CPythonSystem::SetDynamicShadow(bool bEnable)
{
	m_Config.bDynamicShadow = bEnable;
	CPythonBackground::instance().SetDynamicShadow(bEnable);
}
#endif
...
bool CPythonSystem::LoadConfig()
{
	...
	while (fgets(buf, 256, fp))
	{
		...
		else if (!stricmp(command, "WINDOWED"))
		{
...
#ifdef ENABLE_DYNAMIC_SHADOW
		else if (!stricmp(command, "DYNAMIC_SHADOW"))
			m_Config.bDynamicShadow = atoi(value) != 0;
#endif
			...
			m_Config.bFogMode = atoi(value) == 1;
#endif
#if defined(__BL_MULTI_LANGUAGE_ULTIMATE__)
...
#ifdef ENABLE_DYNAMIC_SHADOW
	CPythonBackground::instance().SetDynamicShadow(m_Config.bDynamicShadow);
#endif
