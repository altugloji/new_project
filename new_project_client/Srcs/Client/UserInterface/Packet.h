...
#ifdef GUILD_LARGE_ICON
	MARK_CRC_NUM = GuildMarkDef::BLOCK_TOTAL,
	MARK_DATA_SIZE = GuildMarkDef::MARK_SIZE,
#else
	MARK_CRC_NUM = 8*8,
...
typedef struct command_mark_upload
{
	...
	DWORD	gid;
#ifdef GUILD_LARGE_ICON
	BYTE	image[GuildMarkDef::MARK_PIXEL_BYTES];
#else
	BYTE	image[16*12*4];
#endif
#ifdef ENABLE_GUILD_TOKEN_AUTH
...
typedef struct command_mark_crclist
{
	...
	BYTE	imgIdx;
#ifdef GUILD_LARGE_ICON
	DWORD	crclist[GuildMarkDef::BLOCK_TOTAL];
#else
	DWORD	crclist[80];
#endif
...
enum
{
	...
	// dikkat: block girisleri basta -> mevcut GC degerleri +4 kayar; server packet.h ile BIREBIR ayni sira sart
...
{
#ifdef ENABLE_MESSENGER_BLOCK
	// dikkat: block girisleri basta -> mevcut CG degerleri +3 kayar; server packet.h ile BIREBIR ayni sira sart
...
enum EOfflineShopEditAction
{
	...
	OFFLINE_SHOP_EDIT_ACTION_REMOTE_OPEN = 3,
};
...
// PythonShop.cpp bu sabitleri 'shop' python modulune aktarir.
...
#ifdef ENABLE_LUCKY_DRAW
// server src_server/game/src/packet.h TPacketGCLuckyDrawInfo ile BAYT-BIREBIR ayni olmali
