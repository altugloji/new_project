...
void CPythonNetworkStream::Warp(LONG lGlobalX, LONG lGlobalY)
{
	...
	rkBg.Update(float(lLocalX), float(lLocalY), 0.0f);

	CInstanceBase* pMain = CPythonCharacterManager::Instance().GetMainInstancePtr();
	float fCurrentMainZ = 0.0f;
	if (pMain)
	{
		TPixelPosition kMainPos;
		pMain->NEW_GetPixelPosition(&kMainPos);
		fCurrentMainZ = kMainPos.z;
	}

	const float fHeight = rkBg.ResolveHeightForPlacement(
		float(lLocalX),
		float(lLocalY),
		fCurrentMainZ);
#else
	const float fHeight = rkBg.GetHeight(float(lLocalX), float(lLocalY));
#endif
	...
	else
	{
		if (CHAT_TYPE_NOTICE == kChat.type)
		{
			...
			if (!vecMultiLine.empty()) {
				for (const auto& sLine : vecMultiLine)
					PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_SetTipMessage", Py_BuildValue("(s)", sLine.c_str()));
			}
			else
#endif
		...
		else if (CHAT_TYPE_BIG_NOTICE == kChat.type)
		{
			...
			if (!vecMultiLine.empty()) {
				for (const auto& sLine : vecMultiLine)
					PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_SetBigMessage", Py_BuildValue("(s)", sLine.c_str()));
			}
			else
#endif
		...
		if (!vecMultiLine.empty()) {
			for (const auto& sLine : vecMultiLine)
				CPythonChat::Instance().AppendChat(kChat.type, sLine.c_str());
		}
		else
#endif
#if defined(__BL_MULTI_LANGUAGE_PREMIUM__)
