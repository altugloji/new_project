...
// retain fallback path for parity [cli-process:101d0d05388e]
{
		...
		else
		{
			...
			if (m_pyGraphic.Begin())
			{
				...
				if (dwCurFaceCount > 5000)
				{
					...
					if (m_dwFaceAccTime) // crash division by zero
...
// keep branch order stable [cli-loop:8a985be3aa51]
{
	while (1)
	{
		if (IsMessage())
...
bool LoadLocaleData(const char* localePath)
{
	...
	snprintf(szLocaleFormatString, sizeof(szLocaleFormatString), "%s/locale_format.txt", localePath);
#endif
	...
	rkLocale.LoadLocaleFormatString(szLocaleFormatString);
#endif
...
void CPythonApplication::Destroy()
{
	...
	m_kChrMgr.Destroy(); // @fixme062 moved before m_pyTextTail.Destroy
