...
constexpr float c_fEmpireFlagOffsetY = 4.5f;
...
void CPythonChat::UpdateViewMode(DWORD dwID)
{
	...
	for (itor = pLineList->begin(); itor != pLineList->end();)
	{
...
#endif
		{
			...
			{
				pChatLine->pEmpireFlagImageInstance->SetPosition(pChatSet->m_ix + iWidth, pChatSet->m_iy + iHeight + c_fEmpireFlagOffsetY);
...
void CWhisper::SetBoxSize(float fWidth, float fHeight)
{
	...
	for (auto itor = m_ChatLineDeque.begin(); itor != m_ChatLineDeque.end(); ++itor)
	{
		...
		if (pChatLine->pCountryFlagImageInstance)
#endif
			fNewWidth -= pChatLine->pCountryFlagImageInstance->GetScaledWidth() + 2.0f;
		...
		if (pChatLine->pEmpireFlagImageInstance)
#endif
			fNewWidth -= pChatLine->pEmpireFlagImageInstance->GetScaledWidth() + 2.0f;

		pChatLine->Instance.SetLimitWidth(fNewWidth);
#else
