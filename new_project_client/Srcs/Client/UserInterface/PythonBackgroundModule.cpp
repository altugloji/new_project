...
PyObject * backgroundSetFogMode(PyObject * poSelf, PyObject * poArgs)
{
	...
	CPythonBackground::Instance().SetFogMode(iFlag != 0);
	CPythonSystem::Instance().SetFogMode(iFlag != 0);
	return Py_BuildNone();
}
...
PyObject* backgroundSetDynamicShadow(PyObject* poSelf, PyObject* poArgs)
{
	int iEnable;
	if (!PyTuple_GetInteger(poArgs, 0, &iEnable))
		return Py_BadArgument();

	CPythonBackground::Instance().SetDynamicShadow(iEnable != 0);
	return Py_BuildNone();
}
#endif
...
#ifdef ENABLE_DYNAMIC_SHADOW
		{ "SetDynamicShadow",					backgroundSetDynamicShadow,					METH_VARARGS },
#endif
