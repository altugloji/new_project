...
{
	// @fixme051 BEGIN
	const auto targetRange = rkSkillData.GetTargetRange();
	constexpr auto targetRangeMax = 3500;
	if (rkSkillData.IsNeedBow())
	{
		const auto addonTargetRange = targetRange + GetStatus(POINT_BOW_DISTANCE) * 100;
		return std::min<DWORD>(addonTargetRange, targetRangeMax);

	}
	return targetRange;
	// @fixme051 END (previously: rkSkillData.GetTargetRange() + GetStatus(POINT_BOW_DISTANCE)*100;)
}
