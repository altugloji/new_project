...
public:
	...
	float GetFarClip();
	float GetHeightWithSource(float fx, float fy, bool* pbObjectHeightFound, bool* pbAreaObjectSyncPending) const;
	float ResolveHeightForPlacement(float fx, float fy, float fCurrentZ) const;
	...
	bool IsDynamicShadowEnabled() const { return m_bDynamicShadow; }
#endif
	void RenderSky();
	void RenderCloud();
	void RenderWater();
