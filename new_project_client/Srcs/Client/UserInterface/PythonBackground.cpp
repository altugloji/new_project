...
namespace
{
	constexpr float kPendingDropProtectionThreshold = 50.0f;
}
...
{
	// Force re-apply (map reload resets CMapOutdoor flags via Initialize)
	m_eShadowLevel = -1;
	SetShadowLevel(CPythonSystem::Instance().GetShadowLevel());
#ifdef ENABLE_DYNAMIC_SHADOW
	m_bDynamicShadow = CPythonSystem::Instance().GetDynamicShadow();
	if (m_pkMap)
	{
		CMapOutdoor& rkMap = GetMapOutdoorRef();
		rkMap.SetDrawDynamicShadow(m_bDynamicShadow);
	}
#endif
}
...
float CPythonBackground::GetHeightWithSource(float fx, float fy, bool* pbObjectHeightFound, bool* pbAreaObjectSyncPending) const
{
	if (!m_pkMap)
	{
		if (pbObjectHeightFound)
			*pbObjectHeightFound = false;
		if (pbAreaObjectSyncPending)
			*pbAreaObjectSyncPending = false;
		return 0.0f;
	}

	CMapOutdoor& rkMap = GetMapOutdoorRef();
	return rkMap.GetHeightWithSource(fx, fy, pbObjectHeightFound, pbAreaObjectSyncPending);
}

float CPythonBackground::ResolveHeightForPlacement(float fx, float fy, float fCurrentZ) const
{
	bool bObjectHeightFound = false;
	bool bAreaObjectSyncPending = false;
	const float fResolvedHeight = GetHeightWithSource(fx, fy, &bObjectHeightFound, &bAreaObjectSyncPending);

	const bool bHeightGuarded = (!bObjectHeightFound &&
		bAreaObjectSyncPending &&
		(fCurrentZ - fResolvedHeight) > kPendingDropProtectionThreshold);

	return bHeightGuarded ? fCurrentZ : fResolvedHeight;
}
...
void CPythonBackground::RenderCharacterShadowToTexture()
{
		...
		m_eShadowLevel == SHADOW_GROUND_AND_SOLO)
	{
		...
		if (canRender)
		{
			...
			if (m_bDynamicShadow)
			{
				// Re-establish shadow render states (character rendering may have changed them)
				...
				// Get the light view/projection that BeginRender set on the device
				D3DXMATRIX matLightView, matLightProj;
				STATEMANAGER.GetTransform(D3DTS_VIEW, &matLightView);
				STATEMANAGER.GetTransform(D3DTS_PROJECTION, &matLightProj);
				...
				if (pCamera)
				{
					...
					D3DXVECTOR3 v3LightEye(v3Target.x - 1.732f * SHADOW_BASE,
										   v3Target.y - SHADOW_BASE,
										   v3Target.z + 2.0f * 1.732f * SHADOW_BASE);

					CSpeedTreeDirectX::Instance().UpdateCompundMatrix(v3LightEye, matLightView, matLightProj);
				}

				rkMap.RenderTree(Forest_RenderBranches | Forest_RenderLeaves | Forest_RenderFronds | Forest_RenderToShadow);
...
void CPythonBackground::Warp(DWORD dwX, DWORD dwY)
{
...
#ifdef __BL_FOG_FIX__
	{
		...
		if (m_strMapName == "metin2_map_defensewave")
			bFogMode = true;
		SetFogMode(bFogMode);
	}
#endif
...
#ifdef ENABLE_DYNAMIC_SHADOW
void CPythonBackground::SetDynamicShadow(bool bEnable)
{
	m_bDynamicShadow = bEnable;

	if (IsMapReady())
	{
		CMapOutdoor& rkMap = GetMapOutdoorRef();
		rkMap.SetDrawDynamicShadow(bEnable);
	}
}
#endif
