...
ViewState Frustum::ViewVolumeTest(const Vector3d &c_v3Center,const float c_fRadius) const
{
	...
	const float fExtend = 2.0f;
#else
	const float fExtend = 1.0f;
#endif
	...
	if (m_bUsingSphere)
	{
		...
		if ((c_fRadius * fExtend + m_fRadius) * (c_fRadius * fExtend + m_fRadius) < D3DXVec3LengthSq(&v))
		{
			return VS_OUTSIDE;
		}
	}
	...
	for(i=0;i<count;i++)
	{
		...
		if (distance[i]<=-c_fRadius * fExtend)
			return VS_OUTSIDE;
	}
