...
#if defined(__BL_MULTI_LANGUAGE_PREMIUM__) && defined(__BL_MULTILANGUAGE_CHATTING__)
DWORD CGraphicTextInstance::GetCodePageFromRegion(const char* region)
{
	if (region)
	{
		if (!strncmp(region, "en", 2) ||
			!strncmp(region, "de", 2) ||
			!strncmp(region, "dk", 2) ||
			!strncmp(region, "nl", 2) ||
			!strncmp(region, "fr", 2) ||
		...
		if (!strncmp(region, "pl", 2) ||
			!strncmp(region, "hu", 2) ||
			!strncmp(region, "ro", 2) ||
			!strncmp(region, "cz", 2))
			return CP_EASTEUROPE;

		if (!strncmp(region, "ru", 2))
			return CP_CYRILLIC;

		if (!strncmp(region, "gr", 2))
			return CP_GREEK;

		if (!strncmp(region, "tr", 2))
			return CP_TURKISH;

		if (!strncmp(region, "ae", 2))
			return CP_ARABIC;
	}
...
void CGraphicTextInstance::SetValue(const char* c_szText, DWORD dwCodePage)
#else
