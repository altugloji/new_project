...
CGraphicVertexBuffer CParticleSystemInstance::ms_kParticleVB;

using namespace NEffectUpdateDecorator;

void CParticleSystemInstance::DestroySystem()
{
	ms_kParticleVB.Destroy();
	ms_kPool.Destroy();
...
namespace NParticleRenderer
{
	...
	{
		inline UINT operator () (CParticleInstance * pInstance, TParticleBatchVertex* pDst)
		{
			pInstance->Transform();
			CopyParticleQuad(pDst, pInstance->GetParticleMeshPointer(), pInstance->GetColor());
			return 6;
		}
	};
	...
	struct TwoSideRenderer
	{
		...
		inline UINT operator () (CParticleInstance * pInstance, TParticleBatchVertex* pDst)
		{
			DWORD color = pInstance->GetColor();

			pInstance->Transform(pmat, D3DXToRadian(-30.0f));
			CopyParticleQuad(pDst, pInstance->GetParticleMeshPointer(), color);

			pInstance->Transform(pmat, D3DXToRadian(+30.0f));
			CopyParticleQuad(pDst + 6, pInstance->GetParticleMeshPointer(), color);
	...
	struct ThreeSideRenderer
	{
		...
		inline UINT operator () (CParticleInstance * pInstance, TParticleBatchVertex* pDst)
		{
			...
			pInstance->Transform(pmat, D3DXToRadian(+60.0f));
			CopyParticleQuad(pDst + 12, pInstance->GetParticleMeshPointer(), color);
