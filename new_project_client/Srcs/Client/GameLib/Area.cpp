...
bool CArea::__UpdateLoadedObjectInstances(D3DXVECTOR3& v3Player)
{
	...
	DWORD dwNextIndex = dwStartIndex;

	for (DWORD dwProcessed = 0; dwProcessed < dwObjectCount; ++dwProcessed)
	{
		const DWORD i = (dwStartIndex + dwProcessed) % dwObjectCount;
		TObjectInstance*& rpObjectInstance = m_ObjectInstanceVector[i];
		dwNextIndex = (i + 1) % dwObjectCount;
		...
		if (fMaxDist > fDist)
		{
			if (rpObjectInstance == nullptr)
			{
				rpObjectInstance = ms_ObjectInstancePool.Alloc();
				rpObjectInstance->Clear();

				__SetObjectInstance(rpObjectInstance, c_pObjectData);

				if (rpObjectInstance->dwType == prt::PROPERTY_TYPE_BUILDING)
					m_GraphicThingInstanceCRCMap.emplace(TGraphicThingInstanceCRCMap::value_type(rpObjectInstance->pThingInstance, c_pObjectData->dwCRC));

				Refresh(rpObjectInstance, false);
				bDidModify = true;
			}
		}
		else
		{
			...
			{
				if (rpObjectInstance->dwType == prt::PROPERTY_TYPE_BUILDING)
					m_GraphicThingInstanceCRCMap.erase(rpObjectInstance->pThingInstance);

				Refresh(rpObjectInstance, true);

				__Clear_DestroyObjectInstance(rpObjectInstance);
				rpObjectInstance = nullptr;
				bDidModify = true;
			}
		}
...
void CArea::__UpdateEffectList(D3DXVECTOR3& v3Player)
{
	...
	const float fMaxDist = GetDynamicLoadingDistanceSqr();
...
float CArea::GetDynamicLoadingDistanceSqr() const
{
	int peNum;
...
float CArea::GetGameplayObjectLoadingDistanceSqr() const
{
	int peNum;
	float pfStart, pfEnd, pfFarClip;
	GetDistanceSetInfo(&peNum, &pfStart, &pfEnd, &pfFarClip);

	const float fGameplayLoadDistance = fMAX(pfFarClip, kMinGameplayObjectLoadDistance);
...
void CArea::RenderShadowObjects(const D3DXVECTOR3 & c_rv3Target, float fShadowRangeSq)
{
	for (auto& pObjInst : m_ObjectInstanceVector)
	{
		if (!pObjInst)
			continue;

		if (pObjInst->dwType != prt::PROPERTY_TYPE_BUILDING)
			continue;

		CGraphicThingInstance * pThingInstance = pObjInst->pThingInstance;
		if (!pThingInstance)
			continue;

		// Distance culling: skip buildings outside the shadow ortho projection
		const D3DXVECTOR3 & c_rv3Pos = pThingInstance->GetPosition();
		float fDx = c_rv3Pos.x - c_rv3Target.x;
		float fDy = c_rv3Pos.y - c_rv3Target.y;
		if (fDx * fDx + fDy * fDy > fShadowRangeSq)
			continue;

		// Bridge fix: skip objects whose attribute DATA file defines height surfaces.
		// These are shadow RECEIVERS (bridges/platforms) and would self-darken.
		if (pObjInst->pAttributeInstance)
		{
			CAttributeData * pAttrData = pObjInst->pAttributeInstance->GetObjectPointer();
			if (pAttrData && pAttrData->GetHeightDataCount() > 0)
				continue;
		}

		// Area optimization: objects may not be Refresh'd yet (no Show/DeformAll).
		// Force-deform once -- m_bUpdated stays true for static buildings,
		// so subsequent frames skip straight to RenderToShadowMap.
		// Show() is persistent; CollectRenderingObject uses m_ThingCloneInstaceVector
		// not m_ObjectInstanceVector, so this won't leak into the main render pass.
		if (!pThingInstance->isShow())
		{
			pThingInstance->Update();
			pThingInstance->Transform();
			pThingInstance->DeformAll();
			pThingInstance->Show();
		}

		pThingInstance->RenderToShadowMap();
	}
}
#endif
...
void CArea::Refresh(TObjectInstance* pObjectInstance, bool bRemove)
{
	...
	else if (prt::PROPERTY_TYPE_BUILDING == pObjectInstance->dwType)
	{
		...
		else
		{
			...
			{
#ifdef ENABLE_DYNAMIC_SHADOW
				if (!(pObjectInstance->pAttributeInstance && pObjectInstance->pAttributeInstance->IsHeightData()))
#endif
				{
