...
CGraphicVertexBuffer CWeaponTrace::ms_kVB;
...
void CWeaponTrace::Render()
{
	...
	{
		UINT nVerts = (UINT)m_PDTVertexVector.size();
		if (nVerts > WEAPON_TRACE_MAX_VERTS)
			nVerts = WEAPON_TRACE_MAX_VERTS;

		if (!ms_kVB.GetD3DVertexBuffer())
			ms_kVB.Create(WEAPON_TRACE_MAX_VERTS, D3DFVF_XYZ | D3DFVF_DIFFUSE | D3DFVF_TEX1,
						  D3DUSAGE_DYNAMIC | D3DUSAGE_WRITEONLY, D3DPOOL_DEFAULT);

		TPDTVertex* pVerts;
		if (ms_kVB.GetD3DVertexBuffer() &&
			SUCCEEDED(ms_kVB.GetD3DVertexBuffer()->Lock(0, nVerts * sizeof(TPDTVertex), (void**)&pVerts, D3DLOCK_DISCARD)))
		{
			memcpy(pVerts, &m_PDTVertexVector[0], nVerts * sizeof(TPDTVertex));
			ms_kVB.GetD3DVertexBuffer()->Unlock();
			STATEMANAGER.SetStreamSource(0, ms_kVB.GetD3DVertexBuffer(), sizeof(TPDTVertex));
			STATEMANAGER.DrawPrimitive(D3DPT_TRIANGLESTRIP, 0, nVerts - 2);
		}
	}
