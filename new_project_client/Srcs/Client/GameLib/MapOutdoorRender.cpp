...
void CMapOutdoor::RenderTree(unsigned long ulRenderBitVector)
{
	if (IsVisiblePart(PART_TREE))
		CSpeedTreeForest::Instance().Render(ulRenderBitVector);
}
...
void CMapOutdoor::RenderObjectShadowsToTexture()
{
	...
	static const float SHADOW_HALF_RANGE = 6375.0f; // SHADOW_ORTHO / 2
	static const float SHADOW_RANGE_SQ = SHADOW_HALF_RANGE * SHADOW_HALF_RANGE;
	...
	for (int i = 0; i < AROUND_AREA_NUM; ++i)
	{
		...
		pArea->RenderShadowObjects(v3Target, SHADOW_RANGE_SQ);
	}
}
#endif
...
void CMapOutdoor::SetInverseViewAndDynamicShaodwMatrices()
{
	...
	const D3DXVECTOR3 v3LightEye(v3Target.x - 1.732f * SHADOW_BASE,
	...
	// Snap light-view translation to texel grid - must match BeginRenderCharacterShadowToTexture
	{
#ifdef ENABLE_DYNAMIC_SHADOW
		...
		m_matLightView._41 = floorf(m_matLightView._41 / fTexelSnap) * fTexelSnap;
		m_matLightView._42 = floorf(m_matLightView._42 / fTexelSnap) * fTexelSnap;
	}
