...
void CTerrain::AllocateMarkedSplats(BYTE * pbyAlphaMap)
{
	...
	if (FAILED(ms_lpd3dDevice->CreateTexture(ATTRMAP_XSIZE, ATTRMAP_YSIZE, 1, D3DUSAGE_DYNAMIC, D3DFMT_A8R8G8B8, D3DPOOL_DEFAULT, &m_lpMarkedTexture, nullptr)))
	{
		TraceError("AllocateMarkedSplats() - Failed to CreateTexture");
		return;
	}

	D3DLOCKED_RECT d3dlr;
	if (FAILED(m_lpMarkedTexture->LockRect(0, &d3dlr, nullptr, D3DLOCK_DISCARD)))
	{
		TraceError("AllocateMarkedSplats() - Failed to LockRect");
		safe_release(m_lpMarkedTexture);
		return;
	}
