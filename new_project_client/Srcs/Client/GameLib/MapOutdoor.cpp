...
bool CMapOutdoor::Initialize()
{
	...
	m_bAreaObjectSyncPending = false;
#endif
...
{
	return GetHeightWithSource(fx, fy, nullptr, nullptr);
}

float CMapOutdoor::GetHeightWithSource(float fx, float fy, bool* pbObjectHeightFound, bool* pbAreaObjectSyncPending)
{
	...
	if (!m_bEnableTerrainOnlyForHeight)
	{
		...
		{
			bObjectHeightFound = true;
			fObjectHeight = kGetObjHeight.m_fReturnHeight;
		}

#ifdef ENABLE_AREA_OPTIMIZATION
		bAreaObjectSyncPending = m_bAreaObjectSyncPending;
#endif

		const float fResolvedHeight = fMAX(fObjectHeight, fTerrainHeight);

		if (pbObjectHeightFound)
			*pbObjectHeightFound = bObjectHeightFound;
		if (pbAreaObjectSyncPending)
			*pbAreaObjectSyncPending = bAreaObjectSyncPending;

		return fResolvedHeight;
	}
...
#ifdef ENABLE_DYNAMIC_SHADOW
void CMapOutdoor::SetDrawDynamicShadow(bool bDrawDynamicShadow)
{
	m_bDrawDynamicShadow = bDrawDynamicShadow;
}
#endif
