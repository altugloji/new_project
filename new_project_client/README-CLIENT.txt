...
#define NEW_SELECT_CHARACTER		// enable new select/create ui interface [@warme666]
#define ENABLE_MESSENGER_REMOVE_SYNC	// enable friend remove sync [@warme666]
>>ex
...
#define __BL_FOG_FIX__			// enable fog on/off toggle in system options instead of 3-level fog
#define __BL_FLY_TARGET_POSITION__	// enable per-race fly target adjust position from msm data + mounted height offset
#define __BL_CLIENT_LOCALE_STRING__	// enable server-to-client locale string tags [SN;] [MN;] [IN;] [LS;] [LC;] [LOX;]
...
@fixme051: on UserInterface/PythonPlayerSkill.cpp; official client-side fix for arrow shower skill damage x5 (requires server-side too)
...
@fixme061: on EterLib/GrpMath.h, UserInterface/InstanceBaseBattle.cpp; camera rotation fix - ClampDegree now handles NaN/inf/large values via std::fmod, and dot product is clamped to [-1,1] before acosf to prevent undefined results
@fixme062: on PythonApplication.cpp; destroy m_kChrMgr before m_pyTextTail to prevent use-after-free crash on logout
