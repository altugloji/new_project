...
#define ENABLE_DUNGEON_ELIMINATE_EVENT		// add quest event "dungeon_eliminate"
>>lycan
...
#define ENABLE_NUMBER_ADJUSTMENT		// it fixes the numbers properly without the need to do tonumber("1234567890")
...
@warme018: increased attack range to match client-side value
...
@fixme331: on char_item.cpp; autostack items picked up by other members of the party for you
@fixme332: on mob_manager.cpp; group group had the prob set always to 1
...
@fixme335: on shopEx.cpp; shop ex table may be bigger than the overflow on custom edited projects causing buffer overflow
...
@fixme343: on mob_manager.cpp; memory leak when using /reload or during shutdown; the mob proto elements don't get released properly
