...
bool ITEM_MANAGER::CreateDropItemVector(LPCHARACTER pkChr, LPCHARACTER pkKiller, std::vector<STargetInfoData> & vec_item)
{
	if (!pkChr)
		return false;
...
#ifdef UPDATE_COMMON_DROP_WITH_MAP_INDEX
	...
	while (it != g_vec_pkCommonDropItem[bRank].end())
	{
...
#ifdef UPDATE_COMMON_DROP_WITH_MAP_INDEX
		if (c_rInfo.m_lMapIndex > 0 && c_rInfo.m_lMapIndex != lMapIndex)
			continue;
#endif
		...
		vec_item.emplace_back(STargetInfoData(c_rInfo.m_dwVnum, 1));
/*
		item = NULL;
...
*/
	...
	// Drop Item Group
	{
		...
		if (it != m_map_pkDropItemGroup.end())
		{
			...
			{
				vec_item.emplace_back(STargetInfoData(v[i].dwVnum, v[i].iCount));
/*
...
*/
	...
	if (pkChr->GetMobDropItemVnum())
	{
		...
		{
			vec_item.emplace_back(STargetInfoData(pkChr->GetMobDropItemVnum(), 1));
/*
