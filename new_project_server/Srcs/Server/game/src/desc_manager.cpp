...
static constexpr int MAX_CONNECTION_PER_IP = 20;
static constexpr int MAX_ACCEPT_PER_SEC = 200;
static constexpr int MAX_ACCEPT_PER_IP_PER_SEC = 10;
#endif
...
LPDESC DESC_MANAGER::AcceptDesc(LPFDWATCH fdw, socket_t s)
{
	...
	if (dwNow - s_lastAcceptTime >= 1000)
	{
		s_lastAcceptTime = dwNow;
		s_acceptPerSec = 0;
	}

	if (++s_acceptPerSec > MAX_ACCEPT_PER_SEC)
	{
		sys_err("AcceptDesc: global accept rate exceeded (%d/sec)", s_acceptPerSec);
		socket_close(desc);
		return nullptr;
	}

	// --- Per-IP accept rate limit ---
	IPFloodInfo& rateInfo = s_ipRate[host];

	if (dwNow - rateInfo.lastTime < 1000)
	{
		if (++rateInfo.count > MAX_ACCEPT_PER_IP_PER_SEC)
		{
			sys_err("AcceptDesc: per-IP accept rate exceeded for %s (%d/sec)", host, rateInfo.count);
			socket_close(desc);
			return nullptr;
		}
	}
	...
	{
		rateInfo.count = 1;
		rateInfo.lastTime = dwNow;
	}

	// --- Per-IP connection limit ---
	int& connCount = s_ipConnectionCount[host];

	if (connCount >= MAX_CONNECTION_PER_IP)
	{
		sys_err("AcceptDesc: per-IP connection limit reached for %s (%d/%d)", host, connCount, MAX_CONNECTION_PER_IP);
		socket_close(desc);
		return nullptr;
	}
...
void DESC_MANAGER::DestroyClosed()
{
	...
	while (i != m_set_pkDesc.end())
	{
		...
		{
			if (dwNow - d->GetConnectTime() > 10000)
			{
				sys_err("Handshake timeout for %s", d->GetHostName());
				d->SetPhase(PHASE_CLOSE);
				// Connection will be cleaned up by the logic below synchronously
			}
		}
#endif
