...
#define ENABLE_NUMBER_ADJUSTMENT
...
ostream & operator << (ostream & ostr, const Token& tok)
{
	...
	{
		#ifdef ENABLE_NUMBER_ADJUSTMENT
		double intpart;
		if (std::isfinite(tok.seminfo.r) && std::modf(tok.seminfo.r, &intpart) == 0.0)
			ostr << std::fixed << std::setprecision(0) << tok.seminfo.r;
		else
			ostr << std::defaultfloat << std::setprecision(std::numeric_limits<double>::max_digits10) << tok.seminfo.r;
		ostr << std::defaultfloat << std::setprecision(6); // restore defaults
		#else
		ostr << tok.seminfo.r;
		#endif
	}
