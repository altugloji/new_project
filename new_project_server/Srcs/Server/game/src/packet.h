...
typedef struct packet_safetrade
{
	...
	// INCOMING: ardindan (count) adet TSafeTradeListEntry (tables.h) gelir
} TPacketGCSafeTrade;
#endif
...
#if defined(__BL_MULTI_LANGUAGE_PREMIUM__)
	char	szCountry[COUNTRY_NAME_MAX_LEN + 1];
	packet_chat() : bEmpire(0), bCanFormat(true)
	{
...
typedef struct packet_whisper
{
	...
	BYTE	bEmpire;
	char	szCountry[COUNTRY_NAME_MAX_LEN + 1];
	packet_whisper() : bCanFormat(true), bEmpire(0)
	{
		memset(szCountry, 0, sizeof(szCountry));
	}
#else
...
#ifdef DC_P2P_UPDATE
typedef struct SPacketGGDCP2PUpdate
{
	BYTE	byHeader;
	char	szName[CHARACTER_NAME_MAX_LEN + 1];
