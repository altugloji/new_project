	...
	bool CQuestManager::DND(uint32_t pc, LPITEM item_dnd, LPITEM item_victim, bool bReceiveAll)
	{
		...
		if ((pPC = GetPC(pc)))
		{
			if (!CheckQuestLoaded(pPC))
			{
				...
				if (ch)
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Äù½ºÆ®¸¦ ·ÎµåÇÏ´Â ÁßÀÔ´Ï´Ù. Àá½Ã¸¸ ±â´Ù·Á ÁÖ½Ê½Ã¿À."));
				return false;
			}
			// call script
...
#ifdef ENABLE_DUNGEON_ELIMINATE_EVENT
	bool CQuestManager::DungeonEliminate(uint32_t arg)
	{
		SetServerTimerArg(arg);
		sys_log(0, "XXX Dungeon Eliminate Call NPC %p", GetPCForce(0));
		m_pCurrentPC = GetPCForce(0);
		m_pCurrentCharacter = NULL;
		m_pSelectedDungeon = NULL;
		return m_mapNPC[QUEST_NO_NPC].OnDungeonEliminate(*m_pCurrentPC);
	}
#endif
