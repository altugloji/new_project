...
namespace quest
{
	...
	ALUA(dungeon_set_trigger_at_eliminate)
	{
		auto pDungeon = CQuestManager::instance().GetCurrentDungeon();
		if (pDungeon)
			pDungeon->SetTriggerAtEliminate();
		else
			sys_err("dungeon_set_trigger_at_eliminate: cannot find dungeon");
		return 0;
	}
#endif
	...
	void RegisterDungeonFunctionTable()
	{
		luaL_reg dungeon_functions[] =
		{
			...
			#ifdef ENABLE_DUNGEON_ELIMINATE_EVENT
			{ "set_trigger_at_eliminate",	dungeon_set_trigger_at_eliminate},
			#endif
