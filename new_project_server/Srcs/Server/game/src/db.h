#ifndef __INC_METIN_II_DB_MANAGER_H__
#define __INC_METIN_II_DB_MANAGER_H__

#include "../../libsql/AsyncSQL.h"
#include "any_function.h"
#include <memory>

enum
{
	QUERY_TYPE_RETURN = 1,
	QUERY_TYPE_FUNCTION = 2,
	QUERY_TYPE_AFTER_FUNCTION = 3,
};

enum
{
	QID_SAFEBOX_SIZE,
	QID_AUTH_LOGIN,
	QID_BLOCK_CHAT_LIST,
};

class CQueryInfo
{
	public:
		int	iQueryType;
};

class CReturnQueryInfo : public CQueryInfo
{
	public:
		int	iType;
		DWORD	dwIdent;
		void			*	pvData;
};

class CFuncQueryInfo : public CQueryInfo
{
	public:
		any_function f;
};

class CFuncAfterQueryInfo : public CQueryInfo
{
	public:
		any_void_function f;
};

class CLoginData;

class DBManager : public singleton<DBManager>
{
	public:
		DBManager();
		virtual ~DBManager();

		bool			IsConnected() const;

		bool			Connect(const char * host, const int port, const char * user, const char * pwd, const char * db);
		void			Query(const char * c_pszFormat, ...);

		std::unique_ptr<SQLMsg>		DirectQuery(const char * c_pszFormat, ...);
		void			ReturnQuery(int iType, DWORD dwIdent, void* pvData, const char * c_pszFormat, ...);

		void			Process();
		void			AnalyzeReturnQuery(SQLMsg * pmsg);

		void			SendMoneyLog(BYTE type, DWORD vnum, int gold) const;

		void			LoginPrepare(BYTE bBillType, DWORD dwBillID, long lRemainSecs, LPDESC d, DWORD * pdwClientKey, int * paiPremiumTimes = nullptr);
		void			SendAuthLogin(LPDESC d);
		void			SendLoginPing(const char * c_pszLogin) const;

		void			InsertLoginData(CLoginData * pkLD);
		void			DeleteLoginData(CLoginData * pkLD);
		CLoginData *		GetLoginData(DWORD dwKey);

		DWORD			CountQuery() const { return m_sql.CountQuery(); }
		DWORD			CountQueryResult() const { return m_sql.CountResult(); }
		void			ResetQueryResult()	{ m_sql.ResetQueryFinished(); }

		template<class Functor> void FuncQuery(Functor f, const char * c_pszFormat, ...);
		template<class Functor> void FuncAfterQuery(Functor f, const char * c_pszFormat, ...);

		template<class Functor, typename... Args>
		void FuncSafeQuery(Functor f, const char* tpl, Args&&... args)
		{
			auto query = SqlQuery::Build(m_sql_direct.GetMYSQL(), tpl, std::forward<Args>(args)...);
			const auto p = M2_NEW CFuncQueryInfo;
			p->iQueryType = QUERY_TYPE_FUNCTION;
			p->f = f;
			m_sql.ReturnQuery(query.c_str(), p);
		}

		template<class Functor, typename... Args>
		void FuncAfterSafeQuery(Functor f, const char* tpl, Args&&... args)
		{
			auto query = SqlQuery::Build(m_sql_direct.GetMYSQL(), tpl, std::forward<Args>(args)...);
			const auto p = M2_NEW CFuncAfterQueryInfo;
			p->iQueryType = QUERY_TYPE_AFTER_FUNCTION;
			p->f = f;
			m_sql.ReturnQuery(query.c_str(), p);
		}

		size_t EscapeString(char* dst, size_t dstSize, const char *src, size_t srcSize);

		template<typename... Args>
		void AsyncSafeQuery(const char* tpl, Args&&... args)
		{ m_sql.AsyncQuery(SqlQuery::Build(m_sql_direct.GetMYSQL(), tpl, std::forward<Args>(args)...).c_str()); }

		template<typename... Args>
		void ReturnSafeQuery(int iType, DWORD dwIdent, void* pvData, const char* tpl, Args&&... args)
		{
			auto query = SqlQuery::Build(m_sql_direct.GetMYSQL(), tpl, std::forward<Args>(args)...);
			const auto p = M2_NEW CReturnQueryInfo;
			p->iQueryType = QUERY_TYPE_RETURN;
			p->iType = iType;
			p->dwIdent = dwIdent;
			p->pvData = pvData;
			m_sql.ReturnQuery(query.c_str(), p);
		}

		template<typename... Args>
		std::unique_ptr<SQLMsg> DirectSafeQuery(const char* tpl, Args&&... args)
		{ return m_sql_direct.DirectQuery(SqlQuery::Build(m_sql_direct.GetMYSQL(), tpl, std::forward<Args>(args)...).c_str()); }

	private:
		SQLMsg *				PopResult();

		CAsyncSQL				m_sql;
		CAsyncSQL				m_sql_direct;
		bool					m_bIsConnect;

		std::map<DWORD, CLoginData *>		m_map_pkLoginData;
};

template <class Functor> void DBManager::FuncQuery(Functor f, const char* c_pszFormat, ...)
{
	char szQuery[4096];
	va_list args;

	va_start(args, c_pszFormat);
	vsnprintf(szQuery, 4096, c_pszFormat, args);
	va_end(args);

	const auto p = M2_NEW CFuncQueryInfo;

	p->iQueryType = QUERY_TYPE_FUNCTION;
	p->f = f;

	m_sql.ReturnQuery(szQuery, p);
}

template <class Functor> void DBManager::FuncAfterQuery(Functor f, const char* c_pszFormat, ...)
{
	char szQuery[4096];
	va_list args;

	va_start(args, c_pszFormat);
	vsnprintf(szQuery, 4096, c_pszFormat, args);
	va_end(args);

	const auto p = M2_NEW CFuncAfterQueryInfo;

	p->iQueryType = QUERY_TYPE_AFTER_FUNCTION;
	p->f = f;

	m_sql.ReturnQuery(szQuery, p);
}

////////////////////////////////////////////////////////////////
// ACCOUNT_DB
class AccountDB : public singleton<AccountDB>
{
	public:
		AccountDB();

		bool IsConnected() const;
		bool Connect(const char * host, const int port, const char * user, const char * pwd, const char * db);
		bool ConnectAsync(const char * host, const int port, const char * user, const char * pwd, const char * db, const char * locale);

		std::unique_ptr<SQLMsg> DirectQuery(const char * query);
		void ReturnQuery(int iType, DWORD dwIdent, void * pvData, const char * c_pszFormat, ...);
		void AsyncQuery(const char* query);

		void SetLocale(const std::string & stLocale);

		void Process();

		template<typename... Args>
		void AsyncSafeQuery(const char* tpl, Args&&... args)
		{ m_sql.AsyncQuery(SqlQuery::Build(m_sql_direct.GetMYSQL(), tpl, std::forward<Args>(args)...).c_str()); }

		template<typename... Args>
		std::unique_ptr<SQLMsg> DirectSafeQuery(const char* tpl, Args&&... args)
		{ return m_sql_direct.DirectQuery(SqlQuery::Build(m_sql_direct.GetMYSQL(), tpl, std::forward<Args>(args)...).c_str()); }

	private:
		SQLMsg * PopResult();
		void AnalyzeReturnQuery(SQLMsg * pMsg) const;

		CAsyncSQL2	m_sql_direct;
		CAsyncSQL2	m_sql;
		bool		m_IsConnect;
};
//END_ACCOUNT_DB

#endif
//archive's 6b9a24beef838d9382c750a6b44ccdb4
