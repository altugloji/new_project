...
struct FuncShout
{
	...
	FuncShout(const char* str, BYTE bEmpire, bool bCanFormat, const char* szCountry) : m_str(str), m_bEmpire(bEmpire), m_bCanFormat(bCanFormat), m_szCountry(szCountry)
#else
...
#if defined(__BL_MULTI_LANGUAGE_PREMIUM__)
void SendShout(const char* szText, BYTE bEmpire, bool bCanFormat, const char* szCountry)
#else
#if defined(__BL_CLIENT_LOCALE_STRING__)
...
#if defined(__BL_MULTI_LANGUAGE_PREMIUM__)
	std::for_each(c_ref_set.begin(), c_ref_set.end(), FuncShout(szText, bEmpire, bCanFormat, szCountry));
#else
#if defined(__BL_CLIENT_LOCALE_STRING__)
...
#if defined(__BL_MULTI_LANGUAGE_PREMIUM__)
	SendShout(p->szText, p->bEmpire, p->bCanFormat, p->szCountry);
#else
#if defined(__BL_CLIENT_LOCALE_STRING__)
