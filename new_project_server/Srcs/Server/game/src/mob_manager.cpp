...
void CMobManager::Destroy()
{
	// @fixme343 BEGIN
	// this block is currently disabled because it needs full refactory inside ch->SetProto(pkMob)
	//for (auto& [vnum, pkMob] : m_map_pkMobByVnum)
	//	M2_DELETE(pkMob); // memory leak
	m_map_pkMobByVnum.clear();
	m_map_pkMobByName.clear();

	for (auto& [vnum, pkGroup] : m_map_pkMobGroupGroup)
		M2_DELETE(pkGroup); // memory leak
	m_map_pkMobGroupGroup.clear();

	for (auto& [vnum, pkGroup] : m_map_pkMobGroup)
		M2_DELETE(pkGroup); // memory leak
	...
	for (DWORD i = 0; i < loader.GetChildNodeCount(); ++i)
	{
		...
		for (int k = 1; k < 256; ++k)
		{
			...
			if (loader.GetTokenVector(buf, &pTok))
			{
				...
				if (dwMobVnum)
					pkGroup->AddMember(dwMobVnum, prob); // @fixme332 prob was missing
