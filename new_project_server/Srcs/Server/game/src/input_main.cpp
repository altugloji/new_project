...
void CInputMain::DoBotControl(LPCHARACTER ch, const char* c_pData) {
	if (!ch || !ch->GetDesc()) { return; }
	TPacketCGBotControl* bPacket = (TPacketCGBotControl*)c_pData;
	char szCode[sizeof(bPacket->verifyCode) + 1];
...
int CInputMain::Whisper(LPCHARACTER ch, const char * data, size_t uiBytes) const
{
	...
	// 10 seviye alti oyuncular diger oyunculara PM gonderemez, PM penceresine uyari doner (GM'e gonderim serbest)
	if (ch->GetLevel() < WHISPER_LEVEL_LIMIT && !ch->IsGM() && !bTargetIsGM)
	{
		if (ch->GetDesc())
		{
			char szMsg[128];
			int iLen = snprintf(szMsg, sizeof(szMsg), "Ozel mesaj gonderebilmek icin en az %d seviye olmalisin.", WHISPER_LEVEL_LIMIT);

			if (iLen < 0 || iLen >= (int) sizeof(szMsg))
				iLen = sizeof(szMsg) - 1;
...
#if defined(__BL_MULTI_LANGUAGE_PREMIUM__)
		SendShout(chatbuf, ch->GetEmpire(), p.bCanFormat, p.szCountry);
#else
#if defined(__BL_CLIENT_LOCALE_STRING__)
...
int CInputMain::ShopSearch(LPCHARACTER ch, const char * data, size_t uiBytes) const
{
	...
	// Item-secmeli arama: paketteki secim listesini ilet (dogrulama shop_manager'da)
	std::vector<CShopManager::SOfflineShopFilter> vecSelection;
	{
		size_t count = p->selCount;
		if (count > (size_t)SHOP_SEARCH_SELECT_MAX)
			count = (size_t)SHOP_SEARCH_SELECT_MAX;
		vecSelection.reserve(count);
		for (size_t i = 0; i < count; ++i)
		{
			CShopManager::SOfflineShopFilter kSel;
			kSel.itemVnum = p->sel[i].vnum;
			kSel.socket0 = p->sel[i].socket0;
			vecSelection.push_back(kSel);
		}
	}
	CShopManager::instance().SearchShopItem(ch, p->searchIndex, p->socket0, vecSelection);
#else
		...
		case SHOP_SUBHEADER_CG_SELL2:
			{
				...
				CShopManager::instance().Sell(ch, p->pos, p->count);
