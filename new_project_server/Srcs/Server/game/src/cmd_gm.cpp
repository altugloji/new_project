...
ACMD(do_goto)
{
	...
	if (!*arg1 && !*arg2)
	{
		...
		ch->ChatPacket(CHAT_TYPE_INFO, "Hint: use /goto list to show all available goto locations");
		return;
	}

	if (*arg1 && !strcasecmp(arg1, "list"))
	{
		if (*arg2)
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "Usage: goto list");
			return;
		}

		ch->ChatPacket(CHAT_TYPE_INFO, "Registered goto locations: %u", static_cast<unsigned int>(gs_vec_gotoInfo.size()));

		if (gs_vec_gotoInfo.empty())
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "No goto locations are registered.");
			return;
		}

		for (size_t i = 0; i < gs_vec_gotoInfo.size(); ++i)
		{
			const GotoInfo& c_eachGotoInfo = gs_vec_gotoInfo[i];
			ch->ChatPacket(CHAT_TYPE_INFO, "[%u] name=%s empire=%u mapIndex=%d pos=(%u, %u)",
				static_cast<unsigned int>(i + 1),
				c_eachGotoInfo.st_name.c_str(),
				static_cast<unsigned int>(c_eachGotoInfo.empire),
				c_eachGotoInfo.mapIndex,
				static_cast<unsigned int>(c_eachGotoInfo.x),
				static_cast<unsigned int>(c_eachGotoInfo.y));
		}
