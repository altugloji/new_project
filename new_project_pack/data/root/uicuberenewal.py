	...
	def CountTotal(self):
		...
		for i in xrange(1,6):
			if self.GetVnumMaterial(i) != 0:
				...
				if adet >= self.GetCountMaterial(i):
					number = adet / (self.GetCountMaterial(i))
...
class CubeRenewalWindows(ui.ScriptWindow):
	...
	def __init__(self):
		...
		self.categoria = [
			[1,"BITKI","Ýksir Üretimi"],
			[2,"PARSOMEN","Parþömen Üretimi"],
			[3,"CEVHER","Cevher Üretimi"],
			[4,"EVENT","Event Sandýk Üretimi"],
			[5,"ZIRH","Zýrh Üretimi"],
			[6,"SILAH","Silah Üretimi"],
			[7,"TAKI","Ýç Ýtem Üretimi"],
			[999,"EXTRA", "Diðer Üretimler"],
		]
	...
	def UpdateMaterialCount(self):
		...
		for i in xrange(1,6):
			if self.selectedItem.GetVnumMaterial(i) != 0:
				...
				if adet >= (self.selectedItem.GetCountMaterial(i)*count):
				...
				self.cube_elements["count_elements_%d"%i].SetText("%d/%d"%(adet,self.selectedItem.GetCountMaterial(i)*count))
