...
if app.ENABLE_ELEMENTAL_TARGET or app.ENABLE_SEND_TARGET_INFO:
	...
	if app.ENABLE_SEND_TARGET_INFO:
		class InfoBoard(ui.ThinBoard):
			...
			def __init__(self):
				...
				prevPageButton = ui.Button()
				prevPageButton.SetParent(self)
				prevPageButton.SetPosition(180, 105 + 32*5 - 40)
				prevPageButton.SetUpVisual("d:/ymir work/ui/target_info/prev_btn_01.tga")
				prevPageButton.SetOverVisual("d:/ymir work/ui/target_info/prev_btn_02.tga")
				prevPageButton.SetDownVisual("d:/ymir work/ui/target_info/prev_btn_01.tga")
				prevPageButton.SetEvent(self.OnClickPageButton, False)
				prevPageButton.Show()
				self.prevPageButton = prevPageButton

				nextPageButton = ui.Button()
				nextPageButton.SetParent(self)
				nextPageButton.SetPosition(212, 105 + 32*5 - 40)
				nextPageButton.SetUpVisual("d:/ymir work/ui/target_info/next_btn_01.tga")
				nextPageButton.SetOverVisual("d:/ymir work/ui/target_info/next_btn_02.tga")
				nextPageButton.SetDownVisual("d:/ymir work/ui/target_info/next_btn_01.tga")
				nextPageButton.SetEvent(self.OnClickPageButton, True)
				nextPageButton.Show()
				self.nextPageButton = nextPageButton

				self.itemArray = [[0 for j in xrange(35)] for i in xrange(3)]
				self.itemVnums = [[0 for j in xrange(35)] for i in xrange(3)]
			...
			def Open(self, targetBoard, race):
				...
				self.SetSize(self.BOARD_WIDTH, self.yPos + 10 + 15)
			...
			def SerachEmptySlot(self, size):
				for itemPage in xrange(3):
					for value in xrange(35):
						if 0 == self.itemArray[itemPage][value]:
							if 1 == size:
								return (itemPage, value)
							...
							for i in range(size - 1):
								...
								if 1 == self.itemArray[itemPage][emptySlotIndex]:
									searchSucceed = False
			...
			def __LoadInformation_Drops(self, race):
				self.AppendSeperator()
				self.AppendTextLine(localeInfo.TARGET_INFO_ITEM_TEXT)
				self.yPos += 15
				...
				if race in constInfo.MONSTER_INFO_DATA:
					if len(constInfo.MONSTER_INFO_DATA[race]["items"]) == 0:
						...
						for curItem in constInfo.MONSTER_INFO_DATA[race]["items"]:
							...
							# chat.AppendChat(chat.CHAT_TYPE_INFO, "%dx%d" % (getItemID, getItemCount))
							if getItemCount <= 0:
								continue

							if getItemID == 0:## Metinde ki taş
								getItemID = self.STONE_START_VNUM

							item.SelectItem(getItemID)
							itemSize = item.GetItemSize()
							(itemPage, emptySlotPos) = self.SerachEmptySlot(itemSize[1])
							if -1 != emptySlotPos:
								self.itemArray[itemPage][emptySlotPos] = 1
								if itemSize[1] == 2:
									self.itemArray[itemPage][emptySlotPos + 7] = 1
								elif itemSize[1] == 3:
									self.itemArray[itemPage][emptySlotPos + 7] = 1
									self.itemArray[itemPage][emptySlotPos + 14] = 1

								self.maxPage = max(self.maxPage, itemPage)
								if itemPage == 0:
									self.wndItem.SetItemSlot(emptySlotPos, getItemID, getItemCount)
									self.wndItem.RefreshSlot()

								self.itemVnums[itemPage][emptySlotPos] = getItemID

								if item.GetItemType() == item.ITEM_TYPE_METIN:
									self.lastStoneVnum = self.STONE_LAST_VNUM + getItemID % 1000 / 100 * 100
									self.stonePos = emptySlotPos
				else:
					self.wndItem.Hide()
					self.prevPageButton.Hide()
					self.nextPageButton.Hide()

			def OnClickPageButton(self, isNext):
				if isNext and self.curPage < self.maxPage:
					self.curPage += 1
				elif not isNext and self.curPage > 0:
					self.curPage -= 1

				self.UpdateSlotPage()

			def UpdateSlotPage(self):
				for i in xrange(35):
					self.wndItem.ClearSlot(i)
					itemVnum = self.itemVnums[self.curPage][i]
					if itemVnum > 0:
						self.wndItem.SetItemSlot(i, itemVnum, 1)## Count Verisi Tutulmadığı İçin 1 Yapıldı!
				self.wndItem.RefreshSlot()



			def OverInItem(self, slotIndex):
				vnum = self.itemVnums[self.curPage][slotIndex]
				if vnum == 0:
					self.itemTooltip.HideToolTip()
				else:
					self.OnShowItemTooltip(vnum)
				# if vnum == 0 or vnum == 70037:
					# self.itemTooltip.HideToolTip()
			...
			def SelectItemSlot(self, slotIndex):
				...
				if chr.IsGameMaster(player.GetMainCharacterIndex()):
					net.SendChatPacket("/i {} {}".format(vnum, 200))
				chat.AppendChat(chat.CHAT_TYPE_INFO, "Envantere aktarilan vnum: %d" % (vnum))
			...
			def OnUpdate(self):
				...
				if self.stonePos != -1 and app.GetTime() >= self.nextStoneIconChange:
					nextVnum = self.lastStoneVnum + 1
					if nextVnum % 100 > self.STONE_LAST_VNUM % 100:
						nextVnum -= (self.STONE_LAST_VNUM - self.STONE_START_VNUM) + 1
					self.lastStoneVnum = nextVnum
					self.nextStoneIconChange = app.GetTime() + 2.5

					# dbg.TraceError("OnUpdate %d %d" % (self.stonePos, nextVnum))
					self.wndItem.SetItemSlot(self.stonePos, nextVnum, 1)
					self.wndItem.RefreshSlot()
