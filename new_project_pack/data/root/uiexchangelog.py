...
class ExchangeLog(ui.ScriptWindow):
		...
		def OnRender(self):
			...
			for chilname, childItem in self.ElementDictionary.items():
				...
				if isinstance(childItem, ui.ToggleButton) or isinstance(childItem, ui.Button) or isinstance(childItem, ui.ExpandedImageBox):
					...
					# SetRenderingRect her UI widget'ında yok (ör. Button/ToggleButton).
					# Sadece destekleyen tiplerde kırpma uygula.
					if hasattr(childItem, "SetRenderingRect"):
						if itsRendered:
	...
	def Refresh(self):
		...
		for logData in currentDatList:
			...
			itemY = len(listBox.itemList) * NEXT_Y
			newLog.exPos = (0, itemY)
			newLog.SetPosition(0, itemY)
...
class ScrollBarNew(ui.Window):
	...
	class MiddleBar(ui.DragButton):
		...
		def SetSize(self, height):
			...
			(_, middleY) = self.middle.GetLocalPosition()
			self.bottomScale.SetPosition(0, middleY + self.middle.GetHeight())
