...
class MarkItem(ui.ListBoxEx.Item):
	...
	def __CreateImageBox(self, fileName):
		...
		if 1 == self.canLoad:
			...
			if checkWidth == self.imgWidth and checkHeight == self.imgHeight:
				imgBox = ui.ImageBox()
...
class MarkSelectDialog(ui.ScriptWindow):
	...
	def __OnOK(self):
		...
		if selItem:
			...
			elif selItem.imgWidth != checkWidth:
				if app.GUILD_LARGE_ICON:
					self.__PopupMessage(localeInfo.GUILDMARK_UPLOADER_ERROR_24_WIDTH)
				else:
					self.__PopupMessage(localeInfo.GUILDMARK_UPLOADER_ERROR_16_WIDTH)

			elif selItem.imgHeight != checkHeight:
				if app.GUILD_LARGE_ICON:
					self.__PopupMessage(localeInfo.GUILDMARK_UPLOADER_ERROR_16_HEIGHT)
				else:
