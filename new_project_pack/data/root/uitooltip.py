...
class ItemToolTip(ToolTip):
	...
	def SetInventoryItem(self, slotIndex, window_type = player.INVENTORY):
		...
		if shop.IsOpen():
			if not shop.IsPrivateShop():
				...
				self.__AdjustMaxWidth(attrSlot, item.GetItemDescription()) # @fixme022
				if app.ENABLE_CHEQUE_SYSTEM:
	...
	def HasLastTimeInformation(self, metinSlot):
	...
	def AppendLastTimeInformation(self, metinSlot):
		bHasRealtimeFlag = self.HasLastTimeInformation(metinSlot)
		if bHasRealtimeFlag:
			self.AppendMallItemLastTime(metinSlot[0])
		return bHasRealtimeFlag
